...
- [x] Production 0.1.3 refresh responses carry no-store/no-cache and refreshed public/confidential Bearers initialize MCP 0.1.3 and list all 15 tools
...
Production now makes all discovery surfaces advertise the same approved six-scope grant. The real
Claude Code plugin connector completed its loopback flow and persisted one active all-scope
...
- 2026-07-14 — Pushed `05274de`; Vercel production deployment `dpl_98s1JSu99tMpGs8za8WNRx9ayo5A` became Ready and moved `health.emmetts.dev`. Live checks passed for health, OAuth/OIDC/PRM/JWKS consistency, S256-only discovery, exposed MCP 401 challenge, public and confidential refresh no-store/no-cache headers, refreshed Bearer initialize reporting server 0.1.3, 15-tool discovery, direct Claude Code connectivity, and a post-deploy native Codex read-only tool. Controlled probe cleanup left zero applications, tokens, or consents. (agent: codex)
- 2026-07-31 00:56 - Revalidated the current callback contract against exact hosted Claude,
  Hermes `127.0.0.1:<ephemeral>/callback`, Claude Code `localhost:3118/callback`, and Codex
  `127.0.0.1:<ephemeral>/callback/<server-id>` profiles. The callbacks are accepted; the new
...
- 2026-08-04 - The production Claude Code plugin connector completed DCR/PKCE/consent through a
  dynamic localhost callback and now reports connected. Database evidence shows exactly one active
  rotating refresh credential with the six approved scopes. A separate print-mode agent call chose
